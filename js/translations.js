// Arquivo dedicado para traducoes de legendas e labels em todos os idiomas disponiveis
window.translations = {
	pt: {
		hero_title: "Free Walking Tour",
		hero_subtitle: "Sua próxima grande aventura começa aqui. Descubra destinos surpreendentes e viva roteiros inesquecíveis. <br> Aproveite também nossa experiência exclusiva nos Lençóis Maranhenses.",
		choose_city: "Escolha a cidade:",
		city_rio: "Rio de Janeiro",
		city_salvador: "Salvador",
		city_saoluis: "São Luís do Maranhão",
		city_lencois: "Lençóis Maranhenses",
        nav_home: "INICIO",
		nav_about: "SOBRE",
		nav_contact: "CONTATO",
		nav_help: "AJUDA",
		about_title: "Sobre",
		about_text: "Somos uma equipe apaixonada por experiências locais autênticas, conectando viajantes a guias especialistas em cada destino.",
		contact_title: "Contato",
		contact_text: "Fale com a Travel the World para dúvidas, parcerias e reservas personalizadas.",
		help_title: "Ajuda",
		help_text: "Precisa de suporte com idioma, horários, cancelamentos ou ponto de encontro? Nossa equipe está pronta para ajudar.",
		award_title: "Reconhecimento Especial",
		award_text: "Sabia que fomos premiados como a <b>melhor escolha</b> pelo <b>TripAdvisor</b> em 2021? 🥰",
		search_placeholder: "Pesquisar",
		login: "LOGIN",
		maintenance_title: "Site em manutenção",
		maintenance_body: "Pedimos desculpas pelo transtorno. Estamos com o site temporariamente indisponível e voltaremos em breve. Qualquer dúvida, fale com a nossa equipe:"
	},
	en: {
		hero_title: "Free Walking Tour",
		hero_subtitle: "Discover new destinations, unforgettable experiences & new adventures. <br> Also enjoy our exclusive experience in Lençóis Maranhenses.",
		choose_city: "Choose the city:",
		city_rio: "Rio de Janeiro",
		city_salvador: "Salvador",
		city_saoluis: "São Luís do Maranhão",
		city_lencois: "Lençóis Maranhenses",
		nav_home: "HOME",
		nav_about: "ABOUT",
		nav_contact: "CONTACT",
		nav_help: "HELP",
		about_title: "About",
		about_text: "We are a team passionate about authentic local experiences, connecting travelers with expert guides in each destination.",
		contact_title: "Contact",
		contact_text: "Get in touch with Travel the World for questions, partnerships and personalized bookings.",
		help_title: "Help",
		help_text: "Need support with language, schedules, cancellations or meeting point? Our team is ready to help.",
		award_title: "Special Recognition",
		award_text: "Did you know we were awarded as the <b>best choice</b> by <b>TripAdvisor</b> in 2021? 🥰",
		search_placeholder: "Search",
		login: "LOGIN",
		maintenance_title: "Site under maintenance",
		maintenance_body: "We apologize for the inconvenience. Our site is temporarily unavailable and will be back soon. For any questions, contact our team:"
	},
	fr: {
		hero_title: "Free Walking Tour",
		hero_subtitle: "Découvrez de nouvelles destinations, des expériences inoubliables & de nouvelles aventures. <br> Profitez également de notre expérience exclusive aux Lençóis Maranhenses.",
		choose_city: "Choisissez la ville :",
		city_rio: "Rio de Janeiro",
		city_salvador: "Salvador",
		city_saoluis: "São Luís do Maranhão",
		city_lencois: "Lençóis Maranhenses",
		nav_home: "ACCUEIL",
		nav_about: "À PROPOS",
		nav_contact: "CONTACT",
		nav_help: "AIDE",
		about_title: "À propos",
		about_text: "Nous sommes une équipe passionnée par les expériences locales authentiques, reliant les voyageurs à des guides experts dans chaque destination.",
		contact_title: "Contact",
		contact_text: "Contactez Travel the World pour questions, partenariats et réservations personnalisées.",
		help_title: "Aide",
		help_text: "Besoin d'aide pour la langue, les horaires, les annulations ou le point de rendez-vous ? Notre équipe est prête à vous aider.",
		award_title: "Reconnaissance Spéciale",
		award_text: "Saviez-vous que nous avons été récompensés comme le <b>meilleur choix</b> par <b>TripAdvisor</b> en 2021 ? 🥰",
		search_placeholder: "Rechercher",
		login: "LOGIN",
		maintenance_title: "Site en maintenance",
		maintenance_body: "Nous nous excusons pour la gêne occasionnée. Notre site est temporairement indisponible et sera bientôt de retour. Pour toute question, contactez notre équipe :"
	},
	es: {
		hero_title: "Free Walking Tour",
		hero_subtitle: "Descubre nuevos destinos, experiencias inolvidables & nuevas aventuras. <br> Aprovecha también nuestra experiencia exclusiva en los Lençóis Maranhenses.",
		choose_city: "Elige la ciudad:",
		city_rio: "Rio de Janeiro",
		city_salvador: "Salvador",
		city_saoluis: "São Luís do Maranhão",
		city_lencois: "Lençóis Maranhenses",
		nav_home: "INICIO",
		nav_about: "SOBRE",
		nav_contact: "CONTACTO",
		nav_help: "AYUDA",
		about_title: "Sobre",
		about_text: "Somos un equipo apasionado por experiencias locales auténticas, conectando viajeros con guías expertos en cada destino.",
		contact_title: "Contacto",
		contact_text: "Habla con Travel the World para dudas, alianzas y reservas personalizadas.",
		help_title: "Ayuda",
		help_text: "¿Necesitas ayuda con idioma, horarios, cancelaciones o punto de encuentro? Nuestro equipo está listo para ayudarte.",
		award_title: "Reconocimiento Especial",
		award_text: "¿Sabías que fuimos premiados como la <b>mejor elección</b> por <b>TripAdvisor</b> en 2021? 🥰",
		search_placeholder: "Buscar",
		login: "LOGIN",
		maintenance_title: "Sitio en mantenimiento",
		maintenance_body: "Pedimos disculpas por las molestias. Nuestro sitio está temporalmente no disponible y volverá pronto. Para cualquier duda, contacta a nuestro equipo:"
	},
	it: {
		hero_title: "Free Walking Tour",
		hero_subtitle: "Scopri nuove destinazioni, esperienze indimenticabili & nuove avventure. <br> Approfitta anche della nostra esperienza esclusiva nei Lençóis Maranhenses.",
		choose_city: "Scegli la città:",
		city_rio: "Rio de Janeiro",
		city_salvador: "Salvador",
		city_saoluis: "São Luís do Maranhão",
		city_lencois: "Lençóis Maranhenses",
		nav_home: "INIZIO",
		nav_about: "CHI SIAMO",
		nav_contact: "CONTATTO",
		nav_help: "AIUTO",
		about_title: "Chi siamo",
		about_text: "Siamo un team appassionato di esperienze locali autentiche, che collega i viaggiatori a guide esperte in ogni destinazione.",
		contact_title: "Contatto",
		contact_text: "Contatta Travel the World per domande, partnership e prenotazioni personalizzate.",
		help_title: "Aiuto",
		help_text: "Hai bisogno di supporto con lingua, orari, cancellazioni o punto di incontro? Il nostro team è pronto ad aiutarti.",
		award_title: "Riconoscimento Speciale",
		award_text: "Sapevi che siamo stati premiati come la <b>migliore scelta</b> da <b>TripAdvisor</b> nel 2021? 🥰",
		search_placeholder: "Cerca",
		login: "LOGIN",
		maintenance_title: "Sito in manutenzione",
		maintenance_body: "Ci scusiamo per il disagio. Il nostro sito è temporaneamente non disponibile e tornerà presto online. Per qualsiasi dubbio, contatta il nostro team:"
	},
    zh: {
        hero_title: "徒步旅行团",
        hero_subtitle: "探索令人惊叹的目的地，体验难忘的旅程。<br>还可以体验我们在莱恩索伊斯沙丘的独家行程。",
        choose_city: "选择城市：",
        city_rio: "里约热内卢",
        city_salvador: "萨尔瓦多",
        city_saoluis: "圣路易斯 - 马拉尼昂州",
        city_lencois: "莱恩索伊斯沙丘",
        nav_home: "首页",
        nav_about: "关于我们",
        nav_contact: "联系我们",
        nav_help: "帮助",
        about_title: "关于我们",
        about_text: "我们是一支热衷于原汁原味本地体验的团队，致力于将旅行者与各目的地的专业向导联系起来。",
        contact_title: "联系我们",
        contact_text: "如有疑问、合作意向或需要定制预订，请联系 Travel the World。",
        help_title: "帮助",
        help_text: "需要语言、时间安排、取消或集合地点方面的帮助吗？我们的团队随时为您提供支持。",
        award_title: "特别荣誉",
        award_text: "您知道吗？我们在2021年被<b>TripAdvisor</b>评为<b>最佳选择</b>！",
        search_placeholder: "搜索",
        login: "登录",
        maintenance_title: "网站维护中",
        maintenance_body: "给您带来的不便，我们深表歉意。网站暂时无法访问，我们会尽快恢复。如有任何疑问，请联系我们："
    }
};

window.langMap = {
	pt: { label: "Português", flag: "flag-pt" },
	en: { label: "English", flag: "flag-en" },
	fr: { label: "Français", flag: "flag-fr" },
	es: { label: "Español", flag: "flag-es" },
	it: { label: "Italiano", flag: "flag-it" },
    zh: { label: "Chinese", flag: "flag-zh" }
};

window.pageTranslations = {
Riodejaneiro: {
pt: {
hero_title: 'Free Walking Tour no<br><span class="rio-city-highlight">Rio de Janeiro</span>',
hero_location: 'Rio de Janeiro - Brasil',
hero_desc: 'Seja bem-vindo a cidade Maravilhosa; o Rio dos amores e da beleza inconfundivel te espera de bracos abertos.',
hero_button: 'Conhecer Tours',
notice_title: 'Informacoes Importantes',
notice_lines: [
'Para participar de nosso Free Tour e necessario reservar sua vaga.',
'Seguimos todas as medidas sanitarias exigidas para este tipo de passeio.',
'Por favor estar atentos aos dias disponiveis no formulario de reserva.',
'Sua contribuicao e a remuneracao do guia, seja consciente.',
'Em caso de nao poder comparecer ao tour, cancele sua reserva pelo WhatsApp.'
],
proceed: 'Prosseguir',
section_title: 'Nossas Experiencias',
free_subtitle: 'Free Tours',
paid_title: 'Outros Tours',
paid_subtitle: 'Tours Pagos',
cards: [
{
name: 'Centro Historico',
details: [
'<i class="fa fa-calendar"></i> <strong>Dias:</strong> Segunda a Sabado as 10:30',
'<i class="fa fa-language"></i> <strong>Idiomas:</strong> Portugues, Ingles e Espanhol',
'<i class="fa fa-map-marker-alt"></i> <strong>Encontro:</strong> Praca Cinelandia, em frente ao Theatro Municipal',
'<i class="fa fa-shirt"></i> <strong>Identificacao:</strong> Guias com camisetas verdes'
],
map: '<i class="fa fa-map"></i> Ver no Mapa',
reserve: 'Reservar Agora'
},
{
name: 'Santa Teresa',
details: [
'<i class="fa fa-calendar"></i> <strong>Dias:</strong> Quarta as 15h e Domingo as 11:30',
'<i class="fa fa-language"></i> <strong>Idiomas:</strong> Portugues, Ingles e Espanhol',
'<i class="fa fa-map-marker-alt"></i> <strong>Encontro:</strong> Largo dos Guimaraes, em frente ao Cine Santa Teresa',
'<i class="fa fa-shirt"></i> <strong>Identificacao:</strong> Guias com camisetas verdes'
],
map: '<i class="fa fa-map"></i> Ver no Mapa',
reserve: 'Reservar Agora'
},
{
name: 'Pedra do Sal: Samba e Heranca Afro-brasileira',
details: [
'<i class="fa fa-calendar"></i> <strong>Dias:</strong> Segundas e Sextas as 16h',
'<i class="fa fa-language"></i> <strong>Idiomas:</strong> Portugues, Ingles e Espanhol',
'<i class="fa fa-map-marker-alt"></i> <strong>Encontro:</strong> Museu de Arte do Rio (MAR) - Praca Maua, 5 - Centro',
'<i class="fa fa-flag-checkered"></i> <strong>Finalizacao:</strong> Pedra do Sal (17:30)',
'<i class="fa fa-shirt"></i> <strong>Identificacao:</strong> Guias com camisetas verdes'
],
map: '<i class="fa fa-map"></i> Ver no Mapa',
reserve: 'Reservar Agora'
},
{
name: 'Copacabana e Ipanema',
details: [
'<i class="fa fa-calendar"></i> <strong>Dias:</strong> Quintas as 15:30',
'<i class="fa fa-language"></i> <strong>Idiomas:</strong> Portugues, Ingles e Espanhol',
'<i class="fa fa-map-marker-alt"></i> <strong>Encontro:</strong> Estacao General Osorio (Metro) - Saida A',
'<i class="fa fa-shirt"></i> <strong>Identificacao:</strong> Guias com camisetas verdes'
],
map: '<i class="fa fa-map"></i> Ver no Mapa',
reserve: 'Reservar Agora'
},
{
name: 'Favela Tour <span class="rio-tour-name-sub">(Morro Dona Marta)</span>',
details: [
'<i class="fa fa-calendar"></i> <strong>Dias:</strong> Todos os dias as 14h',
'<i class="fa fa-language"></i> <strong>Idiomas:</strong> Portugues, Ingles e Espanhol',
'<i class="fa fa-tag"></i> <strong>Valor:</strong> R$ 120,00 por pessoa',
'<i class="fa fa-map-marker-alt"></i> <strong>Encontro:</strong> Praca Corumba - Botafogo',
'<i class="fa fa-shirt"></i> <strong>Identificacao:</strong> Guias com camisetas verdes'
],
map: '<i class="fa fa-map"></i> Ver no Mapa',
reserve: 'Reservar Agora'
}
],
footer: 'Travel the World - Experiencias - Rio de Janeiro'
}
},
Salvador: {
pt: {
hero_title: '<span class="rio-city-highlight">Salvador</span>',
hero_location: 'Bahia • Brasil',
hero_desc: 'Descubra a alma vibrante do <span class="rio-hero-accent">Pelourinho</span>: ladeiras coloridas, o som do <span class="rio-hero-accent">berimbau</span> ecoando pelas ruas e séculos de história a cada passo. Mais que um passeio, uma imersão nos <span class="rio-hero-accent">sabores, sons e cores da Bahia</span>.',
hero_button: 'Conhecer Tours',
notice_title: 'Informações Importantes',
notice_lines: [
'Para participar de nosso Free Tour é necessário reservar sua vaga.',
'Seguimos todas as medidas sanitárias exigidas para este tipo de passeio.',
'Por favor estar atentos aos dias disponíveis no formulário de reserva.',
'Sua contribuição é a remuneração do guia, seja consciente.'
],
proceed: 'Prosseguir',
section_title: 'Nossas Experiências',
section_subtitle: 'Free Tours',
tour_name: 'Pelourinho',
details: [
'<i class="fa fa-calendar"></i> <strong>Dias:</strong> Segunda a Domingo às 09:30 · Segunda a Sábado às 15:00',
'<i class="fa fa-language"></i> <strong>Idiomas:</strong> Português, Inglês e Espanhol',
'<i class="fa fa-map-marker-alt"></i> <strong>Encontro:</strong> Em frente ao Cine Metha — Glauber Rocha (Praça Castro Alves)',
'<i class="fa fa-shirt"></i> <strong>Identificação:</strong> Guias com camisetas verdes'
],
map: '<i class="fa fa-map"></i> Ver no Mapa',
reserve: 'Reservar Agora',
depoimentos_title: 'Quem já caminhou com a gente pelo Pelourinho',
testimonials: [
{ author: 'Bruno Castilho', text: 'Guia super atencioso, contou a história do Pelourinho com muita paixão. Recomendo demais!' },
{ author: 'Helena Prado', text: 'Passeio incrível, aprendi muito sobre a cultura baiana. Vale cada minuto do tour.' }
],
relatos_like_label: 'Curtir foto'
},
en: {
hero_title: '<span class="rio-city-highlight">Salvador</span>',
hero_location: 'Bahia • Brazil',
hero_desc: 'On a <span class="rio-hero-accent">Free Walking Tour</span> through the city, your experience goes beyond just seeing. You will be able to <span class="rio-hero-accent">feel, touch and taste</span> a bit of this land\'s <span class="rio-hero-accent">culture and history</span>.',
hero_button: 'See Tours',
notice_title: 'Important Information',
notice_lines: [
'To join our Free Tour you need to reserve your spot.',
'We follow all the sanitary measures required for this type of tour.',
'Please check the available dates on the reservation form.',
'Your contribution is the guide\'s payment, please be conscious.'
],
proceed: 'Proceed',
section_title: 'Our Experiences',
section_subtitle: 'Free Tours',
tour_name: 'Pelourinho',
details: [
'<i class="fa fa-calendar"></i> <strong>Days:</strong> Monday to Sunday at 9:30 AM · Monday to Saturday at 3:00 PM',
'<i class="fa fa-language"></i> <strong>Languages:</strong> Portuguese, English and Spanish',
'<i class="fa fa-map-marker-alt"></i> <strong>Meeting point:</strong> In front of Cine Metha — Glauber Rocha (Castro Alves Square)',
'<i class="fa fa-shirt"></i> <strong>Identification:</strong> Guides wearing green t-shirts'
],
map: '<i class="fa fa-map"></i> View on Map',
reserve: 'Book Now',
depoimentos_title: 'Travelers who explored Pelourinho with us',
testimonials: [
{ author: 'Bruno Castilho', text: 'Very attentive guide, told the story of Pelourinho with real passion. Highly recommend!' },
{ author: 'Helena Prado', text: 'Amazing tour, I learned so much about Bahian culture. Worth every minute.' }
],
relatos_like_label: 'Like photo'
},
es: {
hero_title: '<span class="rio-city-highlight">Salvador</span>',
hero_location: 'Bahía • Brasil',
hero_desc: 'Al hacer un <span class="rio-hero-accent">Free Walking Tour</span> por la ciudad, tu experiencia va más allá de solo ver. Podrás <span class="rio-hero-accent">sentir, tocar y probar</span> un poco de la <span class="rio-hero-accent">cultura y la historia</span> de esta tierra.',
hero_button: 'Conocer Tours',
notice_title: 'Información Importante',
notice_lines: [
'Para participar de nuestro Free Tour es necesario reservar tu lugar.',
'Seguimos todas las medidas sanitarias exigidas para este tipo de paseo.',
'Por favor estar atentos a los días disponibles en el formulario de reserva.',
'Tu contribución es la remuneración del guía, sé consciente.'
],
proceed: 'Continuar',
section_title: 'Nuestras Experiencias',
section_subtitle: 'Free Tours',
tour_name: 'Pelourinho',
details: [
'<i class="fa fa-calendar"></i> <strong>Días:</strong> Lunes a Domingo a las 09:30 · Lunes a Sábado a las 15:00',
'<i class="fa fa-language"></i> <strong>Idiomas:</strong> Portugués, Inglés y Español',
'<i class="fa fa-map-marker-alt"></i> <strong>Encuentro:</strong> Frente al Cine Metha — Glauber Rocha (Plaza Castro Alves)',
'<i class="fa fa-shirt"></i> <strong>Identificación:</strong> Guías con camisetas verdes'
],
map: '<i class="fa fa-map"></i> Ver en el Mapa',
reserve: 'Reservar Ahora',
depoimentos_title: 'Quiénes ya caminaron con nosotros por el Pelourinho',
testimonials: [
{ author: 'Bruno Castilho', text: 'Guía muy atento, contó la historia del Pelourinho con mucha pasión. ¡Lo recomiendo mucho!' },
{ author: 'Helena Prado', text: 'Paseo increíble, aprendí mucho sobre la cultura bahiana. Vale cada minuto del tour.' }
],
relatos_like_label: 'Me gusta'
},
fr: {
hero_title: '<span class="rio-city-highlight">Salvador</span>',
hero_location: 'Bahia • Brésil',
hero_desc: 'En participant à un <span class="rio-hero-accent">Free Walking Tour</span> dans la ville, votre expérience va bien au-delà de la simple observation. Vous pourrez <span class="rio-hero-accent">sentir, toucher et goûter</span> un peu de la <span class="rio-hero-accent">culture et de l\'histoire</span> de cette terre.',
hero_button: 'Découvrir les Tours',
notice_title: 'Informations Importantes',
notice_lines: [
'Pour participer à notre Free Tour, il est nécessaire de réserver votre place.',
'Nous suivons toutes les mesures sanitaires exigées pour ce type de visite.',
'Merci de vérifier les jours disponibles dans le formulaire de réservation.',
'Votre contribution est la rémunération du guide, soyez conscient(e).'
],
proceed: 'Continuer',
section_title: 'Nos Expériences',
section_subtitle: 'Free Tours',
tour_name: 'Pelourinho',
details: [
'<i class="fa fa-calendar"></i> <strong>Jours:</strong> Lundi à Dimanche à 09h30 · Lundi à Samedi à 15h00',
'<i class="fa fa-language"></i> <strong>Langues:</strong> Portugais, Anglais et Espagnol',
'<i class="fa fa-map-marker-alt"></i> <strong>Rendez-vous:</strong> Devant le Cine Metha — Glauber Rocha (Place Castro Alves)',
'<i class="fa fa-shirt"></i> <strong>Identification:</strong> Guides en t-shirt vert'
],
map: '<i class="fa fa-map"></i> Voir sur la Carte',
reserve: 'Réserver Maintenant',
depoimentos_title: 'Ceux qui ont déjà arpenté le Pelourinho avec nous',
testimonials: [
{ author: 'Bruno Castilho', text: 'Guide très attentionné, il a raconté l\'histoire du Pelourinho avec beaucoup de passion. Je recommande vivement !' },
{ author: 'Helena Prado', text: 'Visite incroyable, j\'ai beaucoup appris sur la culture de Bahia. Chaque minute en valait la peine.' }
],
relatos_like_label: "J'aime"
},
it: {
hero_title: '<span class="rio-city-highlight">Salvador</span>',
hero_location: 'Bahia • Brasile',
hero_desc: 'Facendo un <span class="rio-hero-accent">Free Walking Tour</span> in città, la tua esperienza va oltre il semplice vedere. Potrai <span class="rio-hero-accent">sentire, toccare e assaporare</span> un po\' della <span class="rio-hero-accent">cultura e della storia</span> di questa terra.',
hero_button: 'Scopri i Tour',
notice_title: 'Informazioni Importanti',
notice_lines: [
'Per partecipare al nostro Free Tour è necessario prenotare il tuo posto.',
'Seguiamo tutte le misure sanitarie richieste per questo tipo di tour.',
'Presta attenzione ai giorni disponibili nel modulo di prenotazione.',
'Il tuo contributo è il compenso della guida, sii consapevole.'
],
proceed: 'Continua',
section_title: 'Le Nostre Esperienze',
section_subtitle: 'Free Tours',
tour_name: 'Pelourinho',
details: [
'<i class="fa fa-calendar"></i> <strong>Giorni:</strong> Lunedì a Domenica alle 09:30 · Lunedì a Sabato alle 15:00',
'<i class="fa fa-language"></i> <strong>Lingue:</strong> Portoghese, Inglese e Spagnolo',
'<i class="fa fa-map-marker-alt"></i> <strong>Punto d\'incontro:</strong> Davanti al Cine Metha — Glauber Rocha (Piazza Castro Alves)',
'<i class="fa fa-shirt"></i> <strong>Identificazione:</strong> Guide con magliette verdi'
],
map: '<i class="fa fa-map"></i> Vedi sulla Mappa',
reserve: 'Prenota Ora',
depoimentos_title: 'Chi ha già camminato con noi per il Pelourinho',
testimonials: [
{ author: 'Bruno Castilho', text: 'Guida molto attenta, ha raccontato la storia del Pelourinho con grande passione. Consigliatissimo!' },
{ author: 'Helena Prado', text: 'Tour fantastico, ho imparato molto sulla cultura bahiana. Vale ogni minuto.' }
],
relatos_like_label: 'Mi piace'
},
zh: {
hero_title: '<span class="rio-city-highlight">萨尔瓦多</span><span class="rio-city-accent">巴伊亚州</span>',
hero_location: '巴伊亚 • 巴西',
hero_desc: '参加城市<span class="rio-hero-accent">徒步免费游览</span>，您的体验将不仅仅是观赏。您还可以<span class="rio-hero-accent">感受、触摸并品味</span>这片土地的<span class="rio-hero-accent">文化与历史</span>。',
hero_button: '查看行程',
notice_title: '重要信息',
notice_lines: [
'参加我们的免费游览需要预订名额。',
'我们遵守此类游览所需的所有卫生防疫措施。',
'请注意预订表格中提供的可选日期。',
'您的捐赠是导游的报酬，请自觉支付。'
],
proceed: '继续',
section_title: '我们的体验',
section_subtitle: '免费游览',
tour_name: '佩罗林纽',
details: [
'<i class="fa fa-calendar"></i> <strong>时间:</strong> 周一至周日 09:30 · 周一至周六 15:00',
'<i class="fa fa-language"></i> <strong>语言:</strong> 葡萄牙语、英语和西班牙语',
'<i class="fa fa-map-marker-alt"></i> <strong>集合地点:</strong> Cine Metha 影院前 — Glauber Rocha（卡斯特罗·阿尔维斯广场）',
'<i class="fa fa-shirt"></i> <strong>识别标志:</strong> 身穿绿色T恤的导游'
],
map: '<i class="fa fa-map"></i> 查看地图',
reserve: '立即预订',
depoimentos_title: '与我们一起漫步佩罗林纽的旅客们',
testimonials: [
{ author: 'Bruno Castilho', text: '导游非常细心，充满热情地讲述了佩罗林纽的历史。强烈推荐！' },
{ author: 'Helena Prado', text: '精彩的游览，我学到了很多关于巴伊亚文化的知识，非常值得。' }
],
relatos_like_label: '点赞照片'
}
},
Lencoismaranhenses: {
pt: {
hero_title: '<span class="rio-city-highlight">Lençóis Maranhenses</span>',
hero_location: 'Maranhão • Brasil',
hero_desc: 'Descubra o <span class="rio-hero-accent">paraíso das dunas</span> e lagoas cristalinas no coração do <span class="rio-hero-accent">Maranhão</span>. Uma experiência única em um dos cenários naturais mais impressionantes do planeta.',
hero_button: 'Conhecer Tours',
hero_scroll: 'Descubra mais',
notice_title: 'Informações Importantes',
notice_lines: [
'A melhor época para visitar os Lençóis Maranhenses é entre junho e setembro, quando as lagoas estão cheias.',
'Para reservar sua experiência, entre em contato conosco pelo WhatsApp +55 21 97001-8590.',
'Todos os passeios incluem transporte, guia credenciado e seguro.',
'Recomendamos protetor solar, roupas leves, chapéu e calçados confortáveis.'
],
proceed: 'Prosseguir',
section_title: 'Nossas Experiências',
section_subtitle: 'Passeios',
reserve: 'Reservar Agora',
names: ['Lagoas de Santo Amaro', 'Circuito Completo Lençóis'],
card1_details: [
'<i class="fa fa-calendar"></i> <strong>Período:</strong> Junho a Setembro',
'<i class="fa fa-language"></i> <strong>Idiomas:</strong> Português, Inglês e Espanhol',
'<i class="fa fa-map-marker-alt"></i> <strong>Saída:</strong> São Luís ou Barreirinhas',
'<i class="fa fa-users"></i> <strong>Grupo:</strong> Mínimo 2 pessoas'
],
card2_details: [
'<i class="fa fa-calendar"></i> <strong>Período:</strong> Junho a Setembro',
'<i class="fa fa-clock"></i> <strong>Duração:</strong> 3 a 5 dias',
'<i class="fa fa-language"></i> <strong>Idiomas:</strong> Português, Inglês e Espanhol',
'<i class="fa fa-map-marker-alt"></i> <strong>Saída:</strong> São Luís',
'<i class="fa fa-check-circle"></i> <strong>Inclui:</strong> Transporte, hospedagem, guia e refeições'
],
shared_section_title: 'Expedições Compartilhadas',
shared_tours: [
{ name: 'Manhã na Lagoa Azul', details: [
'<i class="fa fa-clock"></i> <strong>Duração:</strong> 8:00 - 14:00',
'<i class="fa fa-map-marker-alt"></i> <strong>Roteiro:</strong> Rio Preguiça, dunas e lagoas circuito Azul',
'<i class="fa fa-check-circle"></i> <strong>Inclui:</strong> Transporte 4x4 e monitor credenciado',
'<i class="fa fa-tag"></i> <strong>Valor:</strong> R$150 por pessoa'
]},
{ name: 'Entardecer na Lagoa Bonita', details: [
'<i class="fa fa-clock"></i> <strong>Duração:</strong> 14:00 - 18:30',
'<i class="fa fa-map-marker-alt"></i> <strong>Roteiro:</strong> Rio Preguiça, dunas e lagoas circuito Bonita',
'<i class="fa fa-check-circle"></i> <strong>Inclui:</strong> Transporte 4x4 e monitor credenciado',
'<i class="fa fa-tag"></i> <strong>Valor:</strong> R$150 por pessoa'
]},
{ name: 'Um dia em Atins Beach', details: [
'<i class="fa fa-clock"></i> <strong>Duração:</strong> 7:00 - 18:30',
'<i class="fa fa-map-marker-alt"></i> <strong>Roteiro:</strong> Rio Preguiça, dunas, lagoas e praia de Atins',
'<i class="fa fa-check-circle"></i> <strong>Inclui:</strong> Transporte 4x4 e monitor credenciado',
'<i class="fa fa-tag"></i> <strong>Valor:</strong> R$230 por pessoa'
]},
{ name: 'Expedição Santo Amaro', details: [
'<i class="fa fa-clock"></i> <strong>Duração:</strong> 7:00 - 19:30',
'<i class="fa fa-map-marker-alt"></i> <strong>Roteiro:</strong> Santo Amaro, dunas e lagoas circuito Betânia',
'<i class="fa fa-check-circle"></i> <strong>Inclui:</strong> Transporte 4x4 e monitor credenciado',
'<i class="fa fa-tag"></i> <strong>Valor:</strong> R$300 por pessoa'
]}
],
private_section_title: 'Expedições Privativas',
private_tours: [
{ name: 'Quadriciclo Adventure', sub: '(Combo lancha + quadri)', details: [
'<i class="fa fa-clock"></i> <strong>Duração:</strong> 7:00 - 18:30',
'<i class="fa fa-map-marker-alt"></i> <strong>Roteiro:</strong> Caburé e Pequenos Lençóis',
'<i class="fa fa-check-circle"></i> <strong>Inclui:</strong> Lancha e quadriciclo com monitor',
'<i class="fa fa-tag"></i> <strong>Valor:</strong> R$750 (2 pessoas)'
]},
{ name: 'Duas Lagoas', sub: '(Azul + Bonita)', details: [
'<i class="fa fa-clock"></i> <strong>Duração:</strong> 8:25 - 17:30',
'<i class="fa fa-map-marker-alt"></i> <strong>Roteiro:</strong> Circuito Azul e circuito Bonita',
'<i class="fa fa-check-circle"></i> <strong>Inclui:</strong> Traslado, guia e visita guiada',
'<i class="fa fa-tag"></i> <strong>Valor:</strong> R$320 por pessoa'
]},
{ name: 'Povoados do Maranhão', sub: '', details: [
'<i class="fa fa-clock"></i> <strong>Duração:</strong> 7:00 - 18:30',
'<i class="fa fa-map-marker-alt"></i> <strong>Roteiro:</strong> Cardosa, Tapuio e circuito de lagoas',
'<i class="fa fa-check-circle"></i> <strong>Inclui:</strong> 4x4, monitor e experiência de farinha artesanal',
'<i class="fa fa-tag"></i> <strong>Valor:</strong> R$1500 (até 8 pessoas)'
]},
{ name: 'Delta das Américas', sub: '', details: [
'<i class="fa fa-clock"></i> <strong>Duração:</strong> 7:00 - 18:30',
'<i class="fa fa-map-marker-alt"></i> <strong>Roteiro:</strong> Tutóia, rios, mar e dunas',
'<i class="fa fa-check-circle"></i> <strong>Inclui:</strong> 4x4 ou executivo, lancha e monitor',
'<i class="fa fa-tag"></i> <strong>Valor:</strong> R$2200 (até 8 pessoas)'
]}
],
reserva_section_title: 'Como realizar minha reserva?',
reserva_steps: [
'1. Fale com nossos consultores no WhatsApp',
'2. Tire todas as dúvidas sobre os roteiros',
'3. Defina datas e passeios',
'4. Realize o sinal de 50% via PIX, boleto ou Paypal',
'5. Receba seu voucher e orientações finais'
],
depoimentos_title: 'A galera que já curtiu os lençóis conosco',
testimonials: [
{ author: 'Maximiliano Osaida', text: 'Bem legal o passeio, em grupo e com guia. A experiência foi segura e muito bem conduzida.' },
{ author: 'Marianna Burlamaqui', text: 'Conheci os locais com outro olhar. A guia explicou tudo e trouxe muita segurança durante o dia.' }
],
relatos_like_label: 'Curtir foto',
footer: 'Travel the World - Experiências - Lençóis Maranhenses'
},
en: {
hero_title: '<span class="rio-city-highlight">Lençóis Maranhenses</span>',
hero_location: 'Maranhão • Brazil',
hero_desc: 'Discover the <span class="rio-hero-accent">paradise of dunes</span> and crystal-clear lagoons in the heart of <span class="rio-hero-accent">Maranhão</span>. A unique experience in one of the most breathtaking natural landscapes on the planet.',
hero_button: 'See Tours',
hero_scroll: 'Discover more',
notice_title: 'Important Information',
notice_lines: [
'The best time to visit Lençóis Maranhenses is between June and September, when the lagoons are full.',
'To book your experience, contact us on WhatsApp +55 21 97001-8590.',
'All tours include transportation, an accredited guide and insurance.',
'We recommend sunscreen, light clothing, a hat and comfortable footwear.'
],
proceed: 'Continue',
section_title: 'Our Experiences',
section_subtitle: 'Tours',
reserve: 'Book Now',
names: ['Santo Amaro Lagoons', 'Full Lençóis Circuit'],
card1_details: [
'<i class="fa fa-calendar"></i> <strong>Season:</strong> June to September',
'<i class="fa fa-language"></i> <strong>Languages:</strong> Portuguese, English and Spanish',
'<i class="fa fa-map-marker-alt"></i> <strong>Departure:</strong> São Luís or Barreirinhas',
'<i class="fa fa-users"></i> <strong>Group:</strong> Minimum 2 people'
],
card2_details: [
'<i class="fa fa-calendar"></i> <strong>Season:</strong> June to September',
'<i class="fa fa-clock"></i> <strong>Duration:</strong> 3 to 5 days',
'<i class="fa fa-language"></i> <strong>Languages:</strong> Portuguese, English and Spanish',
'<i class="fa fa-map-marker-alt"></i> <strong>Departure:</strong> São Luís',
'<i class="fa fa-check-circle"></i> <strong>Includes:</strong> Transportation, lodging, guide and meals'
],
shared_section_title: 'Shared Expeditions',
shared_tours: [
{ name: 'Morning at Blue Lagoon', details: [
'<i class="fa fa-clock"></i> <strong>Duration:</strong> 8:00 AM - 2:00 PM',
'<i class="fa fa-map-marker-alt"></i> <strong>Route:</strong> Preguiças River, dunes and Blue circuit lagoons',
'<i class="fa fa-check-circle"></i> <strong>Includes:</strong> 4x4 transportation and accredited guide',
'<i class="fa fa-tag"></i> <strong>Price:</strong> R$150 per person'
]},
{ name: 'Sunset at Bonita Lagoon', details: [
'<i class="fa fa-clock"></i> <strong>Duration:</strong> 2:00 PM - 6:30 PM',
'<i class="fa fa-map-marker-alt"></i> <strong>Route:</strong> Preguiças River, dunes and Bonita circuit lagoons',
'<i class="fa fa-check-circle"></i> <strong>Includes:</strong> 4x4 transportation and accredited guide',
'<i class="fa fa-tag"></i> <strong>Price:</strong> R$150 per person'
]},
{ name: 'A Day at Atins Beach', details: [
'<i class="fa fa-clock"></i> <strong>Duration:</strong> 7:00 AM - 6:30 PM',
'<i class="fa fa-map-marker-alt"></i> <strong>Route:</strong> Preguiças River, dunes, lagoons and Atins beach',
'<i class="fa fa-check-circle"></i> <strong>Includes:</strong> 4x4 transportation and accredited guide',
'<i class="fa fa-tag"></i> <strong>Price:</strong> R$230 per person'
]},
{ name: 'Santo Amaro Expedition', details: [
'<i class="fa fa-clock"></i> <strong>Duration:</strong> 7:00 AM - 7:30 PM',
'<i class="fa fa-map-marker-alt"></i> <strong>Route:</strong> Santo Amaro, dunes and Betânia circuit lagoons',
'<i class="fa fa-check-circle"></i> <strong>Includes:</strong> 4x4 transportation and accredited guide',
'<i class="fa fa-tag"></i> <strong>Price:</strong> R$300 per person'
]}
],
private_section_title: 'Private Expeditions',
private_tours: [
{ name: 'Quad Bike Adventure', sub: '(Boat + quad bike combo)', details: [
'<i class="fa fa-clock"></i> <strong>Duration:</strong> 7:00 AM - 6:30 PM',
'<i class="fa fa-map-marker-alt"></i> <strong>Route:</strong> Caburé and Pequenos Lençóis',
'<i class="fa fa-check-circle"></i> <strong>Includes:</strong> Boat and quad bike with monitor',
'<i class="fa fa-tag"></i> <strong>Price:</strong> R$750 (2 people)'
]},
{ name: 'Two Lagoons', sub: '(Blue + Bonita)', details: [
'<i class="fa fa-clock"></i> <strong>Duration:</strong> 8:25 AM - 5:30 PM',
'<i class="fa fa-map-marker-alt"></i> <strong>Route:</strong> Blue circuit and Bonita circuit',
'<i class="fa fa-check-circle"></i> <strong>Includes:</strong> Transfer, guide and guided visit',
'<i class="fa fa-tag"></i> <strong>Price:</strong> R$320 per person'
]},
{ name: 'Maranhão Villages', sub: '', details: [
'<i class="fa fa-clock"></i> <strong>Duration:</strong> 7:00 AM - 6:30 PM',
'<i class="fa fa-map-marker-alt"></i> <strong>Route:</strong> Cardosa, Tapuio and lagoon circuit',
'<i class="fa fa-check-circle"></i> <strong>Includes:</strong> 4x4, monitor and artisanal cassava flour experience',
'<i class="fa fa-tag"></i> <strong>Price:</strong> R$1500 (up to 8 people)'
]},
{ name: 'Delta of the Americas', sub: '', details: [
'<i class="fa fa-clock"></i> <strong>Duration:</strong> 7:00 AM - 6:30 PM',
'<i class="fa fa-map-marker-alt"></i> <strong>Route:</strong> Tutóia, rivers, sea and dunes',
'<i class="fa fa-check-circle"></i> <strong>Includes:</strong> 4x4 or executive vehicle, boat and monitor',
'<i class="fa fa-tag"></i> <strong>Price:</strong> R$2200 (up to 8 people)'
]}
],
reserva_section_title: 'How do I make my reservation?',
reserva_steps: [
'1. Talk to our consultants on WhatsApp',
'2. Get all your questions about the itineraries answered',
'3. Choose your dates and tours',
'4. Pay the 50% deposit via PIX, bank transfer or PayPal',
'5. Receive your voucher and final instructions'
],
depoimentos_title: 'People who have already enjoyed the Lençóis with us',
testimonials: [
{ author: 'Maximiliano Osaida', text: 'Really nice tour, in a group and with a guide. The experience was safe and very well conducted.' },
{ author: 'Marianna Burlamaqui', text: 'I got to know the places from a different perspective. The guide explained everything and made the day feel very safe.' }
],
relatos_like_label: 'Like photo',
footer: 'Travel the World - Experiences - Lençóis Maranhenses'
},
es: {
hero_title: '<span class="rio-city-highlight">Lençóis Maranhenses</span>',
hero_location: 'Maranhão • Brasil',
hero_desc: 'Descubre el <span class="rio-hero-accent">paraíso de las dunas</span> y lagunas cristalinas en el corazón de <span class="rio-hero-accent">Maranhão</span>. Una experiencia única en uno de los escenarios naturales más impresionantes del planeta.',
hero_button: 'Ver Tours',
hero_scroll: 'Descubre más',
notice_title: 'Información Importante',
notice_lines: [
'La mejor época para visitar los Lençóis Maranhenses es entre junio y septiembre, cuando las lagunas están llenas.',
'Para reservar tu experiencia, contáctanos por WhatsApp +55 21 97001-8590.',
'Todos los paseos incluyen transporte, guía acreditado y seguro.',
'Recomendamos protector solar, ropa ligera, sombrero y calzado cómodo.'
],
proceed: 'Continuar',
section_title: 'Nuestras Experiencias',
section_subtitle: 'Paseos',
reserve: 'Reservar Ahora',
names: ['Lagunas de Santo Amaro', 'Circuito Completo Lençóis'],
card1_details: [
'<i class="fa fa-calendar"></i> <strong>Temporada:</strong> Junio a Septiembre',
'<i class="fa fa-language"></i> <strong>Idiomas:</strong> Portugués, Inglés y Español',
'<i class="fa fa-map-marker-alt"></i> <strong>Salida:</strong> São Luís o Barreirinhas',
'<i class="fa fa-users"></i> <strong>Grupo:</strong> Mínimo 2 personas'
],
card2_details: [
'<i class="fa fa-calendar"></i> <strong>Temporada:</strong> Junio a Septiembre',
'<i class="fa fa-clock"></i> <strong>Duración:</strong> 3 a 5 días',
'<i class="fa fa-language"></i> <strong>Idiomas:</strong> Portugués, Inglés y Español',
'<i class="fa fa-map-marker-alt"></i> <strong>Salida:</strong> São Luís',
'<i class="fa fa-check-circle"></i> <strong>Incluye:</strong> Transporte, alojamiento, guía y comidas'
],
shared_section_title: 'Expediciones Compartidas',
shared_tours: [
{ name: 'Mañana en la Laguna Azul', details: [
'<i class="fa fa-clock"></i> <strong>Duración:</strong> 8:00 - 14:00',
'<i class="fa fa-map-marker-alt"></i> <strong>Ruta:</strong> Río Preguiças, dunas y lagunas del circuito Azul',
'<i class="fa fa-check-circle"></i> <strong>Incluye:</strong> Transporte 4x4 y monitor acreditado',
'<i class="fa fa-tag"></i> <strong>Precio:</strong> R$150 por persona'
]},
{ name: 'Atardecer en la Laguna Bonita', details: [
'<i class="fa fa-clock"></i> <strong>Duración:</strong> 14:00 - 18:30',
'<i class="fa fa-map-marker-alt"></i> <strong>Ruta:</strong> Río Preguiças, dunas y lagunas del circuito Bonita',
'<i class="fa fa-check-circle"></i> <strong>Incluye:</strong> Transporte 4x4 y monitor acreditado',
'<i class="fa fa-tag"></i> <strong>Precio:</strong> R$150 por persona'
]},
{ name: 'Un día en Atins Beach', details: [
'<i class="fa fa-clock"></i> <strong>Duración:</strong> 7:00 - 18:30',
'<i class="fa fa-map-marker-alt"></i> <strong>Ruta:</strong> Río Preguiças, dunas, lagunas y playa de Atins',
'<i class="fa fa-check-circle"></i> <strong>Incluye:</strong> Transporte 4x4 y monitor acreditado',
'<i class="fa fa-tag"></i> <strong>Precio:</strong> R$230 por persona'
]},
{ name: 'Expedición Santo Amaro', details: [
'<i class="fa fa-clock"></i> <strong>Duración:</strong> 7:00 - 19:30',
'<i class="fa fa-map-marker-alt"></i> <strong>Ruta:</strong> Santo Amaro, dunas y lagunas del circuito Betânia',
'<i class="fa fa-check-circle"></i> <strong>Incluye:</strong> Transporte 4x4 y monitor acreditado',
'<i class="fa fa-tag"></i> <strong>Precio:</strong> R$300 por persona'
]}
],
private_section_title: 'Expediciones Privadas',
private_tours: [
{ name: 'Quadriciclo Adventure', sub: '(Combo lancha + cuatrimoto)', details: [
'<i class="fa fa-clock"></i> <strong>Duración:</strong> 7:00 - 18:30',
'<i class="fa fa-map-marker-alt"></i> <strong>Ruta:</strong> Caburé y Pequenos Lençóis',
'<i class="fa fa-check-circle"></i> <strong>Incluye:</strong> Lancha y cuatrimoto con monitor',
'<i class="fa fa-tag"></i> <strong>Precio:</strong> R$750 (2 personas)'
]},
{ name: 'Dos Lagunas', sub: '(Azul + Bonita)', details: [
'<i class="fa fa-clock"></i> <strong>Duración:</strong> 8:25 - 17:30',
'<i class="fa fa-map-marker-alt"></i> <strong>Ruta:</strong> Circuito Azul y circuito Bonita',
'<i class="fa fa-check-circle"></i> <strong>Incluye:</strong> Traslado, guía y visita guiada',
'<i class="fa fa-tag"></i> <strong>Precio:</strong> R$320 por persona'
]},
{ name: 'Poblados de Maranhão', sub: '', details: [
'<i class="fa fa-clock"></i> <strong>Duración:</strong> 7:00 - 18:30',
'<i class="fa fa-map-marker-alt"></i> <strong>Ruta:</strong> Cardosa, Tapuio y circuito de lagunas',
'<i class="fa fa-check-circle"></i> <strong>Incluye:</strong> 4x4, monitor y experiencia de harina artesanal',
'<i class="fa fa-tag"></i> <strong>Precio:</strong> R$1500 (hasta 8 personas)'
]},
{ name: 'Delta de las Américas', sub: '', details: [
'<i class="fa fa-clock"></i> <strong>Duración:</strong> 7:00 - 18:30',
'<i class="fa fa-map-marker-alt"></i> <strong>Ruta:</strong> Tutóia, ríos, mar y dunas',
'<i class="fa fa-check-circle"></i> <strong>Incluye:</strong> 4x4 o ejecutivo, lancha y monitor',
'<i class="fa fa-tag"></i> <strong>Precio:</strong> R$2200 (hasta 8 personas)'
]}
],
reserva_section_title: '¿Cómo hago mi reserva?',
reserva_steps: [
'1. Habla con nuestros consultores por WhatsApp',
'2. Resuelve todas tus dudas sobre los recorridos',
'3. Define fechas y paseos',
'4. Realiza la seña del 50% vía PIX, transferencia o PayPal',
'5. Recibe tu voucher e instrucciones finales'
],
depoimentos_title: 'La gente que ya disfrutó los lençóis con nosotros',
testimonials: [
{ author: 'Maximiliano Osaida', text: 'Muy lindo el paseo, en grupo y con guía. La experiencia fue segura y muy bien conducida.' },
{ author: 'Marianna Burlamaqui', text: 'Conocí los lugares con otra mirada. La guía explicó todo y transmitió mucha seguridad durante el día.' }
],
relatos_like_label: 'Me gusta',
footer: 'Travel the World - Experiencias - Lençóis Maranhenses'
},
fr: {
hero_title: '<span class="rio-city-highlight">Lençóis Maranhenses</span>',
hero_location: 'Maranhão • Brésil',
hero_desc: 'Découvrez le <span class="rio-hero-accent">paradis des dunes</span> et des lagunes cristallines au cœur du <span class="rio-hero-accent">Maranhão</span>. Une expérience unique dans l\'un des paysages naturels les plus impressionnants de la planète.',
hero_button: 'Voir les Tours',
hero_scroll: 'Découvrir plus',
notice_title: 'Informations Importantes',
notice_lines: [
'La meilleure période pour visiter les Lençóis Maranhenses se situe entre juin et septembre, lorsque les lagunes sont pleines.',
'Pour réserver votre expérience, contactez-nous sur WhatsApp au +55 21 97001-8590.',
'Toutes les excursions incluent le transport, un guide agréé et une assurance.',
'Nous recommandons crème solaire, vêtements légers, chapeau et chaussures confortables.'
],
proceed: 'Continuer',
section_title: 'Nos Expériences',
section_subtitle: 'Excursions',
reserve: 'Réserver',
names: ['Lagunes de Santo Amaro', 'Circuit Complet des Lençóis'],
card1_details: [
'<i class="fa fa-calendar"></i> <strong>Période:</strong> Juin à Septembre',
'<i class="fa fa-language"></i> <strong>Langues:</strong> Portugais, Anglais et Espagnol',
'<i class="fa fa-map-marker-alt"></i> <strong>Départ:</strong> São Luís ou Barreirinhas',
'<i class="fa fa-users"></i> <strong>Groupe:</strong> Minimum 2 personnes'
],
card2_details: [
'<i class="fa fa-calendar"></i> <strong>Période:</strong> Juin à Septembre',
'<i class="fa fa-clock"></i> <strong>Durée:</strong> 3 à 5 jours',
'<i class="fa fa-language"></i> <strong>Langues:</strong> Portugais, Anglais et Espagnol',
'<i class="fa fa-map-marker-alt"></i> <strong>Départ:</strong> São Luís',
'<i class="fa fa-check-circle"></i> <strong>Inclus:</strong> Transport, hébergement, guide et repas'
],
shared_section_title: 'Expéditions Partagées',
shared_tours: [
{ name: 'Matinée au Lagon Bleu', details: [
'<i class="fa fa-clock"></i> <strong>Durée:</strong> 8h00 - 14h00',
'<i class="fa fa-map-marker-alt"></i> <strong>Itinéraire:</strong> Rivière Preguiças, dunes et lagunes du circuit Bleu',
'<i class="fa fa-check-circle"></i> <strong>Inclus:</strong> Transport 4x4 et accompagnateur agréé',
'<i class="fa fa-tag"></i> <strong>Prix:</strong> R$150 par personne'
]},
{ name: 'Coucher de Soleil au Lagon Bonita', details: [
'<i class="fa fa-clock"></i> <strong>Durée:</strong> 14h00 - 18h30',
'<i class="fa fa-map-marker-alt"></i> <strong>Itinéraire:</strong> Rivière Preguiças, dunes et lagunes du circuit Bonita',
'<i class="fa fa-check-circle"></i> <strong>Inclus:</strong> Transport 4x4 et accompagnateur agréé',
'<i class="fa fa-tag"></i> <strong>Prix:</strong> R$150 par personne'
]},
{ name: 'Une Journée à Atins Beach', details: [
'<i class="fa fa-clock"></i> <strong>Durée:</strong> 7h00 - 18h30',
'<i class="fa fa-map-marker-alt"></i> <strong>Itinéraire:</strong> Rivière Preguiças, dunes, lagunes et plage d\'Atins',
'<i class="fa fa-check-circle"></i> <strong>Inclus:</strong> Transport 4x4 et accompagnateur agréé',
'<i class="fa fa-tag"></i> <strong>Prix:</strong> R$230 par personne'
]},
{ name: 'Expédition Santo Amaro', details: [
'<i class="fa fa-clock"></i> <strong>Durée:</strong> 7h00 - 19h30',
'<i class="fa fa-map-marker-alt"></i> <strong>Itinéraire:</strong> Santo Amaro, dunes et lagunes du circuit Betânia',
'<i class="fa fa-check-circle"></i> <strong>Inclus:</strong> Transport 4x4 et accompagnateur agréé',
'<i class="fa fa-tag"></i> <strong>Prix:</strong> R$300 par personne'
]}
],
private_section_title: 'Expéditions Privées',
private_tours: [
{ name: 'Quad Adventure', sub: '(Combo bateau + quad)', details: [
'<i class="fa fa-clock"></i> <strong>Durée:</strong> 7h00 - 18h30',
'<i class="fa fa-map-marker-alt"></i> <strong>Itinéraire:</strong> Caburé et Pequenos Lençóis',
'<i class="fa fa-check-circle"></i> <strong>Inclus:</strong> Bateau et quad avec accompagnateur',
'<i class="fa fa-tag"></i> <strong>Prix:</strong> R$750 (2 personnes)'
]},
{ name: 'Deux Lagons', sub: '(Bleu + Bonita)', details: [
'<i class="fa fa-clock"></i> <strong>Durée:</strong> 8h25 - 17h30',
'<i class="fa fa-map-marker-alt"></i> <strong>Itinéraire:</strong> Circuit Bleu et circuit Bonita',
'<i class="fa fa-check-circle"></i> <strong>Inclus:</strong> Transfert, guide et visite guidée',
'<i class="fa fa-tag"></i> <strong>Prix:</strong> R$320 par personne'
]},
{ name: 'Villages du Maranhão', sub: '', details: [
'<i class="fa fa-clock"></i> <strong>Durée:</strong> 7h00 - 18h30',
'<i class="fa fa-map-marker-alt"></i> <strong>Itinéraire:</strong> Cardosa, Tapuio et circuit des lagunes',
'<i class="fa fa-check-circle"></i> <strong>Inclus:</strong> 4x4, accompagnateur et expérience de farine artisanale',
'<i class="fa fa-tag"></i> <strong>Prix:</strong> R$1500 (jusqu\'à 8 personnes)'
]},
{ name: 'Delta des Amériques', sub: '', details: [
'<i class="fa fa-clock"></i> <strong>Durée:</strong> 7h00 - 18h30',
'<i class="fa fa-map-marker-alt"></i> <strong>Itinéraire:</strong> Tutóia, rivières, mer et dunes',
'<i class="fa fa-check-circle"></i> <strong>Inclus:</strong> 4x4 ou véhicule exécutif, bateau et accompagnateur',
'<i class="fa fa-tag"></i> <strong>Prix:</strong> R$2200 (jusqu\'à 8 personnes)'
]}
],
reserva_section_title: 'Comment faire ma réservation ?',
reserva_steps: [
'1. Parlez à nos conseillers sur WhatsApp',
'2. Posez toutes vos questions sur les itinéraires',
'3. Choisissez vos dates et excursions',
'4. Versez un acompte de 50% via PIX, virement ou PayPal',
'5. Recevez votre bon et les instructions finales'
],
depoimentos_title: 'Ceux qui ont déjà profité des Lençóis avec nous',
testimonials: [
{ author: 'Maximiliano Osaida', text: "Excursion vraiment sympa, en groupe et avec un guide. L'expérience était sûre et très bien encadrée." },
{ author: 'Marianna Burlamaqui', text: "J'ai découvert les lieux sous un autre angle. La guide a tout expliqué et a apporté beaucoup de sécurité tout au long de la journée." }
],
relatos_like_label: "J'aime",
footer: 'Travel the World - Expériences - Lençóis Maranhenses'
},
it: {
hero_title: '<span class="rio-city-highlight">Lençóis Maranhenses</span>',
hero_location: 'Maranhão • Brasile',
hero_desc: 'Scopri il <span class="rio-hero-accent">paradiso delle dune</span> e delle lagune cristalline nel cuore del <span class="rio-hero-accent">Maranhão</span>. Un\'esperienza unica in uno degli scenari naturali più spettacolari del pianeta.',
hero_button: 'Scopri i Tour',
hero_scroll: 'Scopri di più',
notice_title: 'Informazioni Importanti',
notice_lines: [
'Il periodo migliore per visitare i Lençóis Maranhenses è tra giugno e settembre, quando le lagune sono piene.',
'Per prenotare la tua esperienza, contattaci su WhatsApp al +55 21 97001-8590.',
'Tutti i tour includono trasporto, guida autorizzata e assicurazione.',
'Consigliamo crema solare, abiti leggeri, cappello e scarpe comode.'
],
proceed: 'Continua',
section_title: 'Le Nostre Esperienze',
section_subtitle: 'Tour',
reserve: 'Prenota Ora',
names: ['Lagune di Santo Amaro', 'Circuito Completo Lençóis'],
card1_details: [
'<i class="fa fa-calendar"></i> <strong>Periodo:</strong> Giugno a Settembre',
'<i class="fa fa-language"></i> <strong>Lingue:</strong> Portoghese, Inglese e Spagnolo',
'<i class="fa fa-map-marker-alt"></i> <strong>Partenza:</strong> São Luís o Barreirinhas',
'<i class="fa fa-users"></i> <strong>Gruppo:</strong> Minimo 2 persone'
],
card2_details: [
'<i class="fa fa-calendar"></i> <strong>Periodo:</strong> Giugno a Settembre',
'<i class="fa fa-clock"></i> <strong>Durata:</strong> Da 3 a 5 giorni',
'<i class="fa fa-language"></i> <strong>Lingue:</strong> Portoghese, Inglese e Spagnolo',
'<i class="fa fa-map-marker-alt"></i> <strong>Partenza:</strong> São Luís',
'<i class="fa fa-check-circle"></i> <strong>Include:</strong> Trasporto, alloggio, guida e pasti'
],
shared_section_title: 'Spedizioni Condivise',
shared_tours: [
{ name: 'Mattina alla Laguna Azul', details: [
'<i class="fa fa-clock"></i> <strong>Durata:</strong> 8:00 - 14:00',
'<i class="fa fa-map-marker-alt"></i> <strong>Itinerario:</strong> Fiume Preguiças, dune e lagune del circuito Azul',
'<i class="fa fa-check-circle"></i> <strong>Include:</strong> Trasporto 4x4 e accompagnatore autorizzato',
'<i class="fa fa-tag"></i> <strong>Prezzo:</strong> R$150 a persona'
]},
{ name: 'Tramonto alla Laguna Bonita', details: [
'<i class="fa fa-clock"></i> <strong>Durata:</strong> 14:00 - 18:30',
'<i class="fa fa-map-marker-alt"></i> <strong>Itinerario:</strong> Fiume Preguiças, dune e lagune del circuito Bonita',
'<i class="fa fa-check-circle"></i> <strong>Include:</strong> Trasporto 4x4 e accompagnatore autorizzato',
'<i class="fa fa-tag"></i> <strong>Prezzo:</strong> R$150 a persona'
]},
{ name: 'Un Giorno ad Atins Beach', details: [
'<i class="fa fa-clock"></i> <strong>Durata:</strong> 7:00 - 18:30',
'<i class="fa fa-map-marker-alt"></i> <strong>Itinerario:</strong> Fiume Preguiças, dune, lagune e spiaggia di Atins',
'<i class="fa fa-check-circle"></i> <strong>Include:</strong> Trasporto 4x4 e accompagnatore autorizzato',
'<i class="fa fa-tag"></i> <strong>Prezzo:</strong> R$230 a persona'
]},
{ name: 'Spedizione Santo Amaro', details: [
'<i class="fa fa-clock"></i> <strong>Durata:</strong> 7:00 - 19:30',
'<i class="fa fa-map-marker-alt"></i> <strong>Itinerario:</strong> Santo Amaro, dune e lagune del circuito Betânia',
'<i class="fa fa-check-circle"></i> <strong>Include:</strong> Trasporto 4x4 e accompagnatore autorizzato',
'<i class="fa fa-tag"></i> <strong>Prezzo:</strong> R$300 a persona'
]}
],
private_section_title: 'Spedizioni Private',
private_tours: [
{ name: 'Quad Adventure', sub: '(Combo barca + quad)', details: [
'<i class="fa fa-clock"></i> <strong>Durata:</strong> 7:00 - 18:30',
'<i class="fa fa-map-marker-alt"></i> <strong>Itinerario:</strong> Caburé e Pequenos Lençóis',
'<i class="fa fa-check-circle"></i> <strong>Include:</strong> Barca e quad con accompagnatore',
'<i class="fa fa-tag"></i> <strong>Prezzo:</strong> R$750 (2 persone)'
]},
{ name: 'Due Lagune', sub: '(Azul + Bonita)', details: [
'<i class="fa fa-clock"></i> <strong>Durata:</strong> 8:25 - 17:30',
'<i class="fa fa-map-marker-alt"></i> <strong>Itinerario:</strong> Circuito Azul e circuito Bonita',
'<i class="fa fa-check-circle"></i> <strong>Include:</strong> Trasferimento, guida e visita guidata',
'<i class="fa fa-tag"></i> <strong>Prezzo:</strong> R$320 a persona'
]},
{ name: 'Villaggi del Maranhão', sub: '', details: [
'<i class="fa fa-clock"></i> <strong>Durata:</strong> 7:00 - 18:30',
'<i class="fa fa-map-marker-alt"></i> <strong>Itinerario:</strong> Cardosa, Tapuio e circuito delle lagune',
'<i class="fa fa-check-circle"></i> <strong>Include:</strong> 4x4, accompagnatore ed esperienza di farina artigianale',
'<i class="fa fa-tag"></i> <strong>Prezzo:</strong> R$1500 (fino a 8 persone)'
]},
{ name: 'Delta delle Americhe', sub: '', details: [
'<i class="fa fa-clock"></i> <strong>Durata:</strong> 7:00 - 18:30',
'<i class="fa fa-map-marker-alt"></i> <strong>Itinerario:</strong> Tutóia, fiumi, mare e dune',
'<i class="fa fa-check-circle"></i> <strong>Include:</strong> 4x4 o veicolo executive, barca e accompagnatore',
'<i class="fa fa-tag"></i> <strong>Prezzo:</strong> R$2200 (fino a 8 persone)'
]}
],
reserva_section_title: 'Come faccio la mia prenotazione?',
reserva_steps: [
'1. Parla con i nostri consulenti su WhatsApp',
'2. Chiarisci tutti i dubbi sugli itinerari',
'3. Definisci date e tour',
'4. Versa la caparra del 50% tramite PIX, bonifico o PayPal',
'5. Ricevi il tuo voucher e le istruzioni finali'
],
depoimentos_title: 'Chi ha già vissuto i Lençóis con noi',
testimonials: [
{ author: 'Maximiliano Osaida', text: 'Tour molto bello, in gruppo e con guida. L\'esperienza è stata sicura e ben condotta.' },
{ author: 'Marianna Burlamaqui', text: 'Ho conosciuto i luoghi con uno sguardo diverso. La guida ha spiegato tutto e trasmesso molta sicurezza durante la giornata.' }
],
relatos_like_label: 'Mi piace',
footer: 'Travel the World - Esperienze - Lençóis Maranhenses'
},
zh: {
hero_title: '<span class="rio-city-highlight">Lençóis Maranhenses</span>',
hero_location: '马拉尼昂州 • 巴西',
hero_desc: '探索<span class="rio-hero-accent">马拉尼昂州</span>中心的沙丘与清澈泻湖组成的<span class="rio-hero-accent">天堂</span>。这是地球上最令人惊叹的自然景观之一，带给您独一无二的体验。',
hero_button: '查看行程',
hero_scroll: '了解更多',
notice_title: '重要信息',
notice_lines: [
'参观莱恩索伊斯沙丘（Lençóis Maranhenses）的最佳时间是6月至9月，此时泻湖水量充沛。',
'如需预订您的行程，请通过 WhatsApp +55 21 97001-8590 与我们联系。',
'所有行程均包含交通、持证向导及保险。',
'建议携带防晒霜、轻便衣物、帽子及舒适的鞋子。'
],
proceed: '继续',
section_title: '我们的行程',
section_subtitle: '游览项目',
reserve: '立即预订',
names: ['圣阿马罗泻湖', '莱恩索伊斯完整环线'],
card1_details: [
'<i class="fa fa-calendar"></i> <strong>季节:</strong> 6月至9月',
'<i class="fa fa-language"></i> <strong>语言:</strong> 葡萄牙语、英语和西班牙语',
'<i class="fa fa-map-marker-alt"></i> <strong>出发地:</strong> 圣路易斯或巴雷里尼亚斯',
'<i class="fa fa-users"></i> <strong>团队人数:</strong> 最少2人'
],
card2_details: [
'<i class="fa fa-calendar"></i> <strong>季节:</strong> 6月至9月',
'<i class="fa fa-clock"></i> <strong>时长:</strong> 3至5天',
'<i class="fa fa-language"></i> <strong>语言:</strong> 葡萄牙语、英语和西班牙语',
'<i class="fa fa-map-marker-alt"></i> <strong>出发地:</strong> 圣路易斯',
'<i class="fa fa-check-circle"></i> <strong>包含:</strong> 交通、住宿、向导及餐食'
],
shared_section_title: '共享探险',
shared_tours: [
{ name: '蓝湖清晨之旅', details: [
'<i class="fa fa-clock"></i> <strong>时长:</strong> 8:00 - 14:00',
'<i class="fa fa-map-marker-alt"></i> <strong>路线:</strong> 普雷吉萨斯河、沙丘及蓝湖环线泻湖',
'<i class="fa fa-check-circle"></i> <strong>包含:</strong> 4x4交通及持证向导',
'<i class="fa fa-tag"></i> <strong>价格:</strong> 每人150雷亚尔'
]},
{ name: '博尼塔湖日落之旅', details: [
'<i class="fa fa-clock"></i> <strong>时长:</strong> 14:00 - 18:30',
'<i class="fa fa-map-marker-alt"></i> <strong>路线:</strong> 普雷吉萨斯河、沙丘及博尼塔湖环线泻湖',
'<i class="fa fa-check-circle"></i> <strong>包含:</strong> 4x4交通及持证向导',
'<i class="fa fa-tag"></i> <strong>价格:</strong> 每人150雷亚尔'
]},
{ name: '阿廷斯海滩一日游', details: [
'<i class="fa fa-clock"></i> <strong>时长:</strong> 7:00 - 18:30',
'<i class="fa fa-map-marker-alt"></i> <strong>路线:</strong> 普雷吉萨斯河、沙丘、泻湖及阿廷斯海滩',
'<i class="fa fa-check-circle"></i> <strong>包含:</strong> 4x4交通及持证向导',
'<i class="fa fa-tag"></i> <strong>价格:</strong> 每人230雷亚尔'
]},
{ name: '圣阿马罗探险', details: [
'<i class="fa fa-clock"></i> <strong>时长:</strong> 7:00 - 19:30',
'<i class="fa fa-map-marker-alt"></i> <strong>路线:</strong> 圣阿马罗、沙丘及贝塔尼亚环线泻湖',
'<i class="fa fa-check-circle"></i> <strong>包含:</strong> 4x4交通及持证向导',
'<i class="fa fa-tag"></i> <strong>价格:</strong> 每人300雷亚尔'
]}
],
private_section_title: '私人探险',
private_tours: [
{ name: '沙滩摩托车探险', sub: '(快艇+沙滩摩托组合)', details: [
'<i class="fa fa-clock"></i> <strong>时长:</strong> 7:00 - 18:30',
'<i class="fa fa-map-marker-alt"></i> <strong>路线:</strong> 卡布雷及小莱恩索伊斯',
'<i class="fa fa-check-circle"></i> <strong>包含:</strong> 快艇及配向导的沙滩摩托车',
'<i class="fa fa-tag"></i> <strong>价格:</strong> 750雷亚尔（2人）'
]},
{ name: '双湖之旅', sub: '(蓝湖+博尼塔湖)', details: [
'<i class="fa fa-clock"></i> <strong>时长:</strong> 8:25 - 17:30',
'<i class="fa fa-map-marker-alt"></i> <strong>路线:</strong> 蓝湖环线及博尼塔湖环线',
'<i class="fa fa-check-circle"></i> <strong>包含:</strong> 接送、向导及导览',
'<i class="fa fa-tag"></i> <strong>价格:</strong> 每人320雷亚尔'
]},
{ name: '马拉尼昂村落之旅', sub: '', details: [
'<i class="fa fa-clock"></i> <strong>时长:</strong> 7:00 - 18:30',
'<i class="fa fa-map-marker-alt"></i> <strong>路线:</strong> 卡尔多萨、塔皮奥及泻湖环线',
'<i class="fa fa-check-circle"></i> <strong>包含:</strong> 4x4车辆、向导及手工木薯粉制作体验',
'<i class="fa fa-tag"></i> <strong>价格:</strong> 1500雷亚尔（最多8人）'
]},
{ name: '美洲三角洲之旅', sub: '', details: [
'<i class="fa fa-clock"></i> <strong>时长:</strong> 7:00 - 18:30',
'<i class="fa fa-map-marker-alt"></i> <strong>路线:</strong> 图托亚、河流、海洋及沙丘',
'<i class="fa fa-check-circle"></i> <strong>包含:</strong> 4x4或商务车、快艇及向导',
'<i class="fa fa-tag"></i> <strong>价格:</strong> 2200雷亚尔（最多8人）'
]}
],
reserva_section_title: '如何完成预订？',
reserva_steps: [
'1. 通过 WhatsApp 与我们的顾问联系',
'2. 咨询行程的所有疑问',
'3. 确定日期和游览项目',
'4. 通过 PIX、汇款单或 PayPal 支付50%定金',
'5. 收到您的凭证及最终说明'
],
depoimentos_title: '和我们一起体验过莱恩索伊斯的朋友们',
testimonials: [
{ author: 'Maximiliano Osaida', text: '这次团体向导游览非常棒，体验安全且引导得当。' },
{ author: 'Marianna Burlamaqui', text: '我以不同的视角认识了这些地方，向导讲解得很详细，整天都让人感到很安心。' }
],
relatos_like_label: '点赞照片',
footer: 'Travel the World - 体验 - 莱恩索伊斯沙丘'
}
},
Saoluísdomaranhao: {
pt: {
hero_title: '<span class="rio-city-highlight">São Luís</span><span class="rio-city-accent">do Maranhão</span>',
hero_location: 'Maranhão • Brasil',
hero_desc: 'Perca-se pelos <span class="rio-hero-accent">azulejos centenários</span> e ruas de pedra da <span class="rio-hero-accent">Atenas Brasileira</span>, porta de entrada para o paraíso natural dos Lençóis Maranhenses. Um centro histórico <span class="rio-hero-accent">Patrimônio da Humanidade</span> que encanta a cada esquina.',
hero_button: 'Conhecer Tour',
notice_title: 'Informações Importantes',
notice_lines: [
'Para participar de nosso Free Tour é necessário reservar sua vaga.',
'Seguimos todas as medidas sanitárias exigidas para este tipo de passeio.',
'Por favor estar atentos aos dias disponíveis no formulário de reserva.',
'Sua contribuição é a remuneração do guia, seja consciente.'
],
proceed: 'Prosseguir',
section_title: 'Nossas Experiências',
free_subtitle: 'Free Tours',
paid_title: 'Outros Tours',
paid_subtitle: 'Tours Privados',
names: ['Centro Histórico', 'Tour Privado <span class="rio-tour-name-sub">(Centro Histórico)</span>'],
card1_details: [
'<i class="fa fa-clock"></i> <strong>Duração:</strong> 2h15',
'<i class="fa fa-calendar"></i> <strong>Dias:</strong> Todos os dias (Jul–Set) às 10:00 e 15:30 (PT) · Qua e Sáb às 10:00 (ES)',
'<i class="fa fa-language"></i> <strong>Idiomas:</strong> Português e Espanhol',
'<i class="fa fa-map-marker-alt"></i> <strong>Encontro:</strong> Cais da Praia Grande, Av. Sen. Vitorino Freire — Centro',
'<i class="fa fa-shirt"></i> <strong>Identificação:</strong> Guias com camisetas verdes'
],
card2_details: [
'<i class="fa fa-clock"></i> <strong>Duração:</strong> 3h00',
'<i class="fa fa-calendar"></i> <strong>Dias:</strong> Todos os dias às 10:00, 13:00 e 15:00',
'<i class="fa fa-language"></i> <strong>Idiomas:</strong> Português, Inglês e Espanhol',
'<i class="fa fa-tag"></i> <strong>Valor:</strong> R$ 250,00 (1–3 pessoas) · R$ 80,00/pessoa (4+)',
'<i class="fa fa-map-marker-alt"></i> <strong>Encontro:</strong> Cais da Praia Grande, Av. Sen. Vitorino Freire — Centro'
],
card1_map: '<i class="fa fa-map"></i> Ver no Mapa',
card1_reserve: 'Reservar Agora',
card2_map: '<i class="fa fa-map"></i> Ver no Mapa',
card2_reserve: 'Reservar Agora',
depoimentos_title: 'Quem já explorou o centro histórico conosco',
testimonials: [
{ author: 'Marcelo Andrade', text: 'Guia excelente, conhece cada detalhe do casario colonial. Uma aula de história a céu aberto.' },
{ author: 'Patrícia Nogueira', text: 'Tour maravilhoso pelo centro histórico, azulejos lindos e muita cultura. Recomendo a todos!' }
],
relatos_like_label: 'Curtir foto'
},
en: {
hero_title: '<span class="rio-city-highlight">São Luís</span><span class="rio-city-accent">do Maranhão</span>',
hero_location: 'Maranhão • Brazil',
hero_desc: 'Welcome to the capital of <span class="rio-hero-accent">Maranhão</span>; besides being the gateway to the natural paradise of <span class="rio-hero-accent">Lençóis Maranhenses</span>, São Luís has one of the most <span class="rio-hero-accent">emblematic historic centers</span> in the world.',
hero_button: 'See Tours',
notice_title: 'Important Information',
notice_lines: [
'To join our Free Tour you need to reserve your spot.',
'We follow all the sanitary measures required for this type of tour.',
'Please check the available dates on the reservation form.',
'Your contribution is the guide\'s payment, please be conscious.'
],
proceed: 'Proceed',
section_title: 'Our Experiences',
free_subtitle: 'Free Tours',
paid_title: 'Other Tours',
paid_subtitle: 'Private Tours',
names: ['Historic Center', 'Private Tour <span class="rio-tour-name-sub">(Historic Center)</span>'],
card1_details: [
'<i class="fa fa-clock"></i> <strong>Duration:</strong> 2h15',
'<i class="fa fa-calendar"></i> <strong>Days:</strong> Every day (Jul–Sep) at 10:00 AM and 3:30 PM (PT) · Wed and Sat at 10:00 AM (ES)',
'<i class="fa fa-language"></i> <strong>Languages:</strong> Portuguese and Spanish',
'<i class="fa fa-map-marker-alt"></i> <strong>Meeting point:</strong> Cais da Praia Grande, Av. Sen. Vitorino Freire — Downtown',
'<i class="fa fa-shirt"></i> <strong>Identification:</strong> Guides wearing green t-shirts'
],
card2_details: [
'<i class="fa fa-clock"></i> <strong>Duration:</strong> 3h00',
'<i class="fa fa-calendar"></i> <strong>Days:</strong> Every day at 10:00 AM, 1:00 PM and 3:00 PM',
'<i class="fa fa-language"></i> <strong>Languages:</strong> Portuguese, English and Spanish',
'<i class="fa fa-tag"></i> <strong>Price:</strong> R$ 250.00 (1–3 people) · R$ 80.00/person (4+)',
'<i class="fa fa-map-marker-alt"></i> <strong>Meeting point:</strong> Cais da Praia Grande, Av. Sen. Vitorino Freire — Downtown'
],
card1_map: '<i class="fa fa-map"></i> View on Map',
card1_reserve: 'Book Now',
card2_map: '<i class="fa fa-map"></i> View on Map',
card2_reserve: 'Book Now',
depoimentos_title: 'Travelers who explored the historic center with us',
testimonials: [
{ author: 'Marcelo Andrade', text: 'Excellent guide, knows every detail of the colonial buildings. Like an open-air history class.' },
{ author: 'Patrícia Nogueira', text: 'Wonderful tour through the historic center, beautiful tiles and lots of culture. Highly recommend!' }
],
relatos_like_label: 'Like photo'
},
es: {
hero_title: '<span class="rio-city-highlight">São Luís</span><span class="rio-city-accent">do Maranhão</span>',
hero_location: 'Maranhão • Brasil',
hero_desc: 'Bienvenido a la <span class="rio-hero-accent">capital ludovicense</span>; además de ser la puerta de entrada al paraíso natural de los <span class="rio-hero-accent">Lençóis Maranhenses</span>, São Luís posee uno de los <span class="rio-hero-accent">centros históricos más emblemáticos</span> y encantadores del mundo.',
hero_button: 'Conocer Tours',
notice_title: 'Información Importante',
notice_lines: [
'Para participar de nuestro Free Tour es necesario reservar tu lugar.',
'Seguimos todas las medidas sanitarias exigidas para este tipo de paseo.',
'Por favor estar atentos a los días disponibles en el formulario de reserva.',
'Tu contribución es la remuneración del guía, sé consciente.'
],
proceed: 'Continuar',
section_title: 'Nuestras Experiencias',
free_subtitle: 'Free Tours',
paid_title: 'Otros Tours',
paid_subtitle: 'Tours Privados',
names: ['Centro Histórico', 'Tour Privado <span class="rio-tour-name-sub">(Centro Histórico)</span>'],
card1_details: [
'<i class="fa fa-clock"></i> <strong>Duración:</strong> 2h15',
'<i class="fa fa-calendar"></i> <strong>Días:</strong> Todos los días (Jul–Sep) a las 10:00 y 15:30 (PT) · Mié y Sáb a las 10:00 (ES)',
'<i class="fa fa-language"></i> <strong>Idiomas:</strong> Portugués y Español',
'<i class="fa fa-map-marker-alt"></i> <strong>Encuentro:</strong> Cais da Praia Grande, Av. Sen. Vitorino Freire — Centro',
'<i class="fa fa-shirt"></i> <strong>Identificación:</strong> Guías con camisetas verdes'
],
card2_details: [
'<i class="fa fa-clock"></i> <strong>Duración:</strong> 3h00',
'<i class="fa fa-calendar"></i> <strong>Días:</strong> Todos los días a las 10:00, 13:00 y 15:00',
'<i class="fa fa-language"></i> <strong>Idiomas:</strong> Portugués, Inglés y Español',
'<i class="fa fa-tag"></i> <strong>Precio:</strong> R$ 250,00 (1–3 personas) · R$ 80,00/persona (4+)',
'<i class="fa fa-map-marker-alt"></i> <strong>Encuentro:</strong> Cais da Praia Grande, Av. Sen. Vitorino Freire — Centro'
],
card1_map: '<i class="fa fa-map"></i> Ver en el Mapa',
card1_reserve: 'Reservar Ahora',
card2_map: '<i class="fa fa-map"></i> Ver en el Mapa',
card2_reserve: 'Reservar Ahora',
depoimentos_title: 'Quiénes ya exploraron el centro histórico con nosotros',
testimonials: [
{ author: 'Marcelo Andrade', text: 'Guía excelente, conoce cada detalle del caserío colonial. Una clase de historia al aire libre.' },
{ author: 'Patrícia Nogueira', text: 'Tour maravilloso por el centro histórico, azulejos hermosos y mucha cultura. ¡Lo recomiendo a todos!' }
],
relatos_like_label: 'Me gusta'
},
fr: {
hero_title: '<span class="rio-city-highlight">São Luís</span><span class="rio-city-accent">do Maranhão</span>',
hero_location: 'Maranhão • Brésil',
hero_desc: 'Bienvenue dans la <span class="rio-hero-accent">capitale ludovicienne</span> ; en plus d\'être la porte d\'entrée du paradis naturel des <span class="rio-hero-accent">Lençóis Maranhenses</span>, São Luís possède l\'un des <span class="rio-hero-accent">centres historiques les plus emblématiques</span> et charmants du monde.',
hero_button: 'Découvrir les Tours',
notice_title: 'Informations Importantes',
notice_lines: [
'Pour participer à notre Free Tour, il est nécessaire de réserver votre place.',
'Nous suivons toutes les mesures sanitaires exigées pour ce type de visite.',
'Merci de vérifier les jours disponibles dans le formulaire de réservation.',
'Votre contribution est la rémunération du guide, soyez conscient(e).'
],
proceed: 'Continuer',
section_title: 'Nos Expériences',
free_subtitle: 'Free Tours',
paid_title: 'Autres Tours',
paid_subtitle: 'Tours Privés',
names: ['Centre Historique', 'Tour Privé <span class="rio-tour-name-sub">(Centre Historique)</span>'],
card1_details: [
'<i class="fa fa-clock"></i> <strong>Durée:</strong> 2h15',
'<i class="fa fa-calendar"></i> <strong>Jours:</strong> Tous les jours (Juil–Sep) à 10h00 et 15h30 (PT) · Mer et Sam à 10h00 (ES)',
'<i class="fa fa-language"></i> <strong>Langues:</strong> Portugais et Espagnol',
'<i class="fa fa-map-marker-alt"></i> <strong>Rendez-vous:</strong> Cais da Praia Grande, Av. Sen. Vitorino Freire — Centre',
'<i class="fa fa-shirt"></i> <strong>Identification:</strong> Guides en t-shirt vert'
],
card2_details: [
'<i class="fa fa-clock"></i> <strong>Durée:</strong> 3h00',
'<i class="fa fa-calendar"></i> <strong>Jours:</strong> Tous les jours à 10h00, 13h00 et 15h00',
'<i class="fa fa-language"></i> <strong>Langues:</strong> Portugais, Anglais et Espagnol',
'<i class="fa fa-tag"></i> <strong>Prix:</strong> R$ 250,00 (1–3 personnes) · R$ 80,00/personne (4+)',
'<i class="fa fa-map-marker-alt"></i> <strong>Rendez-vous:</strong> Cais da Praia Grande, Av. Sen. Vitorino Freire — Centre'
],
card1_map: '<i class="fa fa-map"></i> Voir sur la Carte',
card1_reserve: 'Réserver Maintenant',
card2_map: '<i class="fa fa-map"></i> Voir sur la Carte',
card2_reserve: 'Réserver Maintenant',
depoimentos_title: 'Ceux qui ont déjà exploré le centre historique avec nous',
testimonials: [
{ author: 'Marcelo Andrade', text: 'Guide excellent, il connaît chaque détail des bâtiments coloniaux. Un vrai cours d\'histoire en plein air.' },
{ author: 'Patrícia Nogueira', text: 'Visite merveilleuse du centre historique, azulejos magnifiques et beaucoup de culture. Je recommande à tous !' }
],
relatos_like_label: "J'aime"
},
it: {
hero_title: '<span class="rio-city-highlight">São Luís</span><span class="rio-city-accent">do Maranhão</span>',
hero_location: 'Maranhão • Brasile',
hero_desc: 'Benvenuti nella <span class="rio-hero-accent">capitale ludovicense</span>; oltre a essere la porta d\'ingresso al paradiso naturale dei <span class="rio-hero-accent">Lençóis Maranhenses</span>, São Luís possiede uno dei <span class="rio-hero-accent">centri storici più emblematici</span> e affascinanti del mondo.',
hero_button: 'Scopri i Tour',
notice_title: 'Informazioni Importanti',
notice_lines: [
'Per partecipare al nostro Free Tour è necessario prenotare il tuo posto.',
'Seguiamo tutte le misure sanitarie richieste per questo tipo di tour.',
'Presta attenzione ai giorni disponibili nel modulo di prenotazione.',
'Il tuo contributo è il compenso della guida, sii consapevole.'
],
proceed: 'Continua',
section_title: 'Le Nostre Esperienze',
free_subtitle: 'Free Tours',
paid_title: 'Altri Tour',
paid_subtitle: 'Tour Privati',
names: ['Centro Storico', 'Tour Privato <span class="rio-tour-name-sub">(Centro Storico)</span>'],
card1_details: [
'<i class="fa fa-clock"></i> <strong>Durata:</strong> 2h15',
'<i class="fa fa-calendar"></i> <strong>Giorni:</strong> Tutti i giorni (Lug–Set) alle 10:00 e 15:30 (PT) · Mer e Sab alle 10:00 (ES)',
'<i class="fa fa-language"></i> <strong>Lingue:</strong> Portoghese e Spagnolo',
'<i class="fa fa-map-marker-alt"></i> <strong>Punto d\'incontro:</strong> Cais da Praia Grande, Av. Sen. Vitorino Freire — Centro',
'<i class="fa fa-shirt"></i> <strong>Identificazione:</strong> Guide con magliette verdi'
],
card2_details: [
'<i class="fa fa-clock"></i> <strong>Durata:</strong> 3h00',
'<i class="fa fa-calendar"></i> <strong>Giorni:</strong> Tutti i giorni alle 10:00, 13:00 e 15:00',
'<i class="fa fa-language"></i> <strong>Lingue:</strong> Portoghese, Inglese e Spagnolo',
'<i class="fa fa-tag"></i> <strong>Prezzo:</strong> R$ 250,00 (1–3 persone) · R$ 80,00/persona (4+)',
'<i class="fa fa-map-marker-alt"></i> <strong>Punto d\'incontro:</strong> Cais da Praia Grande, Av. Sen. Vitorino Freire — Centro'
],
card1_map: '<i class="fa fa-map"></i> Vedi sulla Mappa',
card1_reserve: 'Prenota Ora',
card2_map: '<i class="fa fa-map"></i> Vedi sulla Mappa',
card2_reserve: 'Prenota Ora',
depoimentos_title: 'Chi ha già esplorato il centro storico con noi',
testimonials: [
{ author: 'Marcelo Andrade', text: 'Guida eccellente, conosce ogni dettaglio degli edifici coloniali. Una lezione di storia a cielo aperto.' },
{ author: 'Patrícia Nogueira', text: 'Tour meraviglioso nel centro storico, azulejos bellissimi e tanta cultura. Consigliatissimo a tutti!' }
],
relatos_like_label: 'Mi piace'
},
zh: {
hero_title: '<span class="rio-city-highlight">圣路易斯</span><span class="rio-city-accent">马拉尼昂州</span>',
hero_location: '马拉尼昂州 • 巴西',
hero_desc: '欢迎来到<span class="rio-hero-accent">路多维森斯之都</span>；除了是<span class="rio-hero-accent">伦索伊斯马拉年斯</span>自然天堂的门户外，圣路易斯还拥有世界上<span class="rio-hero-accent">最具代表性</span>和迷人魅力的历史中心之一。',
hero_button: '查看行程',
notice_title: '重要信息',
notice_lines: [
'参加我们的免费游览需要预订名额。',
'我们遵守此类游览所需的所有卫生防疫措施。',
'请注意预订表格中提供的可选日期。',
'您的捐赠是导游的报酬，请自觉支付。'
],
proceed: '继续',
section_title: '我们的体验',
free_subtitle: '免费游览',
paid_title: '其他行程',
paid_subtitle: '私人行程',
names: ['历史中心', '私人游览 <span class="rio-tour-name-sub">(历史中心)</span>'],
card1_details: [
'<i class="fa fa-clock"></i> <strong>时长:</strong> 2小时15分钟',
'<i class="fa fa-calendar"></i> <strong>时间:</strong> 每天（7月–9月）10:00 和 15:30（葡语）· 周三和周六 10:00（西语）',
'<i class="fa fa-language"></i> <strong>语言:</strong> 葡萄牙语和西班牙语',
'<i class="fa fa-map-marker-alt"></i> <strong>集合地点:</strong> Cais da Praia Grande，Av. Sen. Vitorino Freire — 市中心',
'<i class="fa fa-shirt"></i> <strong>识别标志:</strong> 身穿绿色T恤的导游'
],
card2_details: [
'<i class="fa fa-clock"></i> <strong>时长:</strong> 3小时',
'<i class="fa fa-calendar"></i> <strong>时间:</strong> 每天 10:00、13:00 和 15:00',
'<i class="fa fa-language"></i> <strong>语言:</strong> 葡萄牙语、英语和西班牙语',
'<i class="fa fa-tag"></i> <strong>价格:</strong> R$ 250,00（1-3人）· 每人 R$ 80,00（4人以上）',
'<i class="fa fa-map-marker-alt"></i> <strong>集合地点:</strong> Cais da Praia Grande，Av. Sen. Vitorino Freire — 市中心'
],
card1_map: '<i class="fa fa-map"></i> 查看地图',
card1_reserve: '立即预订',
card2_map: '<i class="fa fa-map"></i> 查看地图',
card2_reserve: '立即预订',
depoimentos_title: '与我们一起探索历史中心的旅客们',
testimonials: [
{ author: 'Marcelo Andrade', text: '导游非常出色，熟知殖民建筑的每一个细节，就像一堂露天历史课。' },
{ author: 'Patrícia Nogueira', text: '历史中心之旅非常精彩，瓷砖漂亮，文化底蕴深厚，强烈推荐！' }
],
relatos_like_label: '点赞照片'
}
}
};

(function ensureAllCityLanguages() {
    const langs = ['en', 'fr', 'es', 'it', 'zh'];

    const phraseMap = {
        en: {
            'Informações Importantes': 'Important Information',
            'Nossas Experiências': 'Our Experiences',
            'Outros Tours': 'Other Tours',
            'Tours Pagos': 'Paid Tours',
            'Tours Privados': 'Private Tours',
            'Prosseguir': 'Proceed',
            'Conhecer Tours': 'See Tours',
            'Reservar Agora': 'Book Now',
            'Ver no Mapa': 'View on Map',
            'Mais Detalhes': 'More Details',
            'Reservar via WhatsApp': 'Book via WhatsApp',
            'Duração': 'Duration',
            'Dias': 'Days',
            'Idiomas': 'Languages',
            'Encontro': 'Meeting',
            'Identificação': 'Identification',
            'Finalização': 'Finish',
            'Valor': 'Price',
            'Período': 'Season',
            'Saída': 'Departure',
            'Grupo': 'Group',
            'Inclui': 'Includes'
        },
        fr: {
            'Informações Importantes': 'Informations importantes',
            'Nossas Experiências': 'Nos experiences',
            'Outros Tours': 'Autres tours',
            'Tours Pagos': 'Tours payants',
            'Tours Privados': 'Tours prives',
            'Prosseguir': 'Continuer',
            'Conhecer Tours': 'Voir les tours',
            'Reservar Agora': 'Reserver',
            'Ver no Mapa': 'Voir la carte',
            'Mais Detalhes': 'Plus de details',
            'Reservar via WhatsApp': 'Reserver via WhatsApp',
            'Duração': 'Duree',
            'Dias': 'Jours',
            'Idiomas': 'Langues',
            'Encontro': 'Rendez-vous',
            'Identificação': 'Identification',
            'Finalização': 'Fin',
            'Valor': 'Prix',
            'Período': 'Periode',
            'Saída': 'Depart',
            'Grupo': 'Groupe',
            'Inclui': 'Comprend'
        },
        es: {
            'Informações Importantes': 'Informacion importante',
            'Nossas Experiências': 'Nuestras experiencias',
            'Outros Tours': 'Otros tours',
            'Tours Pagos': 'Tours pagados',
            'Tours Privados': 'Tours privados',
            'Prosseguir': 'Continuar',
            'Conhecer Tours': 'Ver tours',
            'Reservar Agora': 'Reservar ahora',
            'Ver no Mapa': 'Ver en el mapa',
            'Mais Detalhes': 'Mas detalles',
            'Reservar via WhatsApp': 'Reservar por WhatsApp',
            'Duração': 'Duracion',
            'Dias': 'Dias',
            'Idiomas': 'Idiomas',
            'Encontro': 'Encuentro',
            'Identificação': 'Identificacion',
            'Finalização': 'Final',
            'Valor': 'Valor',
            'Período': 'Periodo',
            'Saída': 'Salida',
            'Grupo': 'Grupo',
            'Inclui': 'Incluye'
        },
        it: {
            'Informações Importantes': 'Informazioni importanti',
            'Nossas Experiências': 'Le nostre esperienze',
            'Outros Tours': 'Altri tours',
            'Tours Pagos': 'Tours a pagamento',
            'Tours Privados': 'Tours privati',
            'Prosseguir': 'Procedi',
            'Conhecer Tours': 'Vedi tours',
            'Reservar Agora': 'Prenota ora',
            'Ver no Mapa': 'Vedi sulla mappa',
            'Mais Detalhes': 'Maggiori dettagli',
            'Reservar via WhatsApp': 'Prenota via WhatsApp',
            'Duração': 'Durata',
            'Dias': 'Giorni',
            'Idiomas': 'Lingue',
            'Encontro': 'Incontro',
            'Identificação': 'Identificazione',
            'Finalização': 'Fine',
            'Valor': 'Prezzo',
            'Período': 'Periodo',
            'Saída': 'Partenza',
            'Grupo': 'Gruppo',
            'Inclui': 'Include'
        },
        zh: {
            'Informações Importantes': '重要信息',
            'Nossas Experiências': '我们的行程',
            'Outros Tours': '其他行程',
            'Tours Pagos': '付费行程',
            'Tours Privados': '私人行程',
            'Prosseguir': '继续',
            'Conhecer Tours': '查看行程',
            'Reservar Agora': '立即预订',
            'Ver no Mapa': '查看地图',
            'Mais Detalhes': '更多详情',
            'Reservar via WhatsApp': '通过WhatsApp预订',
            'Duração': '时长',
            'Dias': '天数',
            'Idiomas': '语言',
            'Encontro': '集合地点',
            'Identificação': '识别标志',
            'Finalização': '结束',
            'Valor': '价格',
            'Período': '季节',
            'Saída': '出发地',
            'Grupo': '团队人数',
            'Inclui': '包含'
        }
    };

    function translateValue(value, lang) {
        if (typeof value !== 'string') return value;
        let out = value;
        const map = phraseMap[lang] || {};
        for (const [pt, tr] of Object.entries(map)) {
            out = out.replaceAll(pt, tr);
        }
        return out;
    }

    function deepTranslate(obj, lang) {
        if (Array.isArray(obj)) return obj.map((v) => deepTranslate(v, lang));
        if (obj && typeof obj === 'object') {
            const out = {};
            for (const [k, v] of Object.entries(obj)) out[k] = deepTranslate(v, lang);
            return out;
        }
        return translateValue(obj, lang);
    }

    if (!window.pageTranslations) return;

    for (const pageKey of Object.keys(window.pageTranslations)) {
        const page = window.pageTranslations[pageKey];
        if (!page || typeof page !== 'object' || !page.pt) continue;
        for (const lang of langs) {
            if (!page[lang]) {
                page[lang] = deepTranslate(page.pt, lang);
            }
        }
    }
})();

